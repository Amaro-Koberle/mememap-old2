...
import { QUERY } from 'src/components/Node/NodesCell'

const DELETE_NODE_MUTATION = gql`
  mutation DeleteNodeMutation($id: Int!) {
    deleteNode(id: $id) {
      id
    }
  }
    ...
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        ...
        <tbody>
          {nodes.map((node) => (
            <tr key={node.id}>
              <td>{truncate(node.id)}</td>
              <td>{timeTag(node.createdAt)}</td>
              <td>{truncate(node.title)}</td>
              <td>{truncate(node.content)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.node({ id: node.id })}
                    title={'Show node ' + node.id + ' detail'}
